...
export async function handler(event) {
  ...
  try {
    ...
    if (favoritos.includes(String(productId))) {
      return { statusCode: 200, body: JSON.stringify({ success: true, message: "Ya está en favoritos", favoritos }) };
    }

    // Añadir a favoritos
    favoritos.push(String(productId));
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({ success: true, message: "Añadido a favoritos", favoritos })
    };
